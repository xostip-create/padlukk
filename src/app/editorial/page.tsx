    ...
    <div className="bg-background min-h-screen font-body">
      <div className="container mx-auto max-w-6xl py-16 md:py-24 px-4">
        ...
        <div className="text-center mb-20 space-y-6">
          <h1 className="font-headline text-5xl md:text-7xl lg:text-8xl tracking-tighter">Editorial</h1>
        ...
        {!loading && !error && posts && posts.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-24">
            {posts.map((post, index) => (
              ...
              >
                <div className={cn(
                    "relative overflow-hidden aspect-[16/10] bg-muted",
                  ...
                  <div className="absolute top-6 left-6">
                    <span className="bg-background/80 backdrop-blur-md px-4 py-1 text-[10px] tracking-[0.2em] uppercase font-bold border border-white/10">
                        {post.category}
                    </span>
                  </div>
                </div>
        ...
        {!loading && !error && (!posts || posts.length === 0) && (
            <div className="text-center py-32 border-t border-border/20">
                <p className="text-muted-foreground font-headline text-2xl italic">The archives are quiet. Content follows the moment.</p>
            </div>
        )}
      </div>
    </div>
